...
const ERROR_MAP: Record<string, { title: string; description: string }> = {
  Configuration: {
    ...
    description:
      "There is a problem with the server configuration. Check if your environment variables (NEXTAUTH_SECRET, ATLAS_API_KEY, NEXT_PUBLIC_API_URL) are set in Cloudflare dashboard.",
  },
  AccessDenied: {
    title: "Access Denied",
    description:
  ...
  RefreshAccessTokenError: {
    ...
    description:
      "Your session token could not be refreshed. Please sign in again to continue.",
  },
  Default: {
    ...
    >
      ...
      <Card className="ambient-shadow border border-[rgba(0,0,0,0.06)] bg-white p-2">
        <CardHeader className="pb-2 text-center">
          ...
          <CardTitle className="text-on-surface text-xl font-semibold">
            {errorInfo.title}
          </CardTitle>
        ...
        <CardContent className="pt-2">
          <div className="bg-surface-container-low text-muted-foreground rounded-lg p-3 text-center font-mono text-xs">
            Error Code:{" "}
            <span className="text-on-surface font-semibold">{errorType}</span>
          </div>
        </CardContent>
